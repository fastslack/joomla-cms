...
class JOauth2Credentials
{
	...
	/**
	 * @var    JOauth2TableCredentials  Connector object for table class.
	 * @since  1.0
	 */
	public $_table;

	/**
	 * @var    JOauth2CredentialsState  The current credential state.
	 * @since  1.0
	 */
	public $_state;

	/**
	 * @var    JOauth2ProtocolRequest   The current HTTP request.
	 * @since  1.0
	 */
	public $_request;
	 ...
	 * Object constructor.
	 *
	 * @param   JOauth2ProtocolRequest   $request  The HTTP request
	 * @param   JOauth2TableCredentials  $table    Connector object for table class.
	 *
	 * @since   1.0
	 */
	public function __construct(JOauth2ProtocolRequest $request, JOauth2TableCredentials $table = null)
	{
		// Load the HTTP request
		$this->_request = $request ? $request : new JOauth2ProtocolRequest;

		// Setup the database object.
		$this->_table = $table ? $table : JTable::getInstance('Credentials', 'JOauth2Table');

		// Assume the base state for any credentials object to be new.
		$this->_state = new JOauth2CredentialsStateNew($this->_table);

		// Setup the correct signer
		$signature = isset($this->_request->signature_method) ? $this->_request->signature_method : 'PLAINTEXT';
		$this->_signer = JOauth2CredentialsSigner::getInstance($signature);
	}
	 ...
	 *
	 * @param   JOauth2Client  $client  The client.
	 *
	 * @return  boolean  True if authentication is ok, false if not
	 *
	 * @since   1.0
	 */
	public function doJoomlaAuthentication(JOauth2Client $client)
	{
